...
# Load data
url = 'https://raw.githubusercontent.com/sr2322/SugarcaneNIR/main/Nirscan_nano.CSV'
data = pd.read_csv(url, sep=';')
X = data.values[:,1:]
wl = data.columns[1:] #1071 not included
wl = [float(numeric_string) for numeric_string in wl]
print (wl)

# Load reference data (to get total sugar values)
SampleNums = data.values[:,0]
...
with plt.style.context(('ggplot')):
    ...
    ax2 = plt.subplot(313)
    for Xsnv_i, TS_i in zip(Xsnv,SampleTSes):
       if TS_i != 0:
            plt.plot(wl, Xsnv_i, lw=1, c=cm.hot( 1-(TS_i/TS_max)) )
    # plt.plot(wl, Xsnv.T)
